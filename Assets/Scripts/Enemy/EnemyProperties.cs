...
public class EnemyProperties : MonoBehaviour
{
	...
	[SerializeField] private bool isDeathEffect = false;
	...
	private void DropGoldDie()
	{
		GameManager.Instance.AddGold(gold);
		if (isDeathEffect)
			enemyManager?.StopMovementForSeconds(3);
		Die();
	}
	...
	void OnCollisionEnter2D(Collision2D collision)
	{
		if (collision.gameObject.CompareTag("Bullet"))
		{
			...
			TakeDamage(collision.gameObject.GetComponent<BulletController>().BaseDamage * UpgradeStats.Instance.damageMultiplier);
		}
